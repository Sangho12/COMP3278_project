...
def feed():
  ...
  # 🔥🔥🔥 EDITED START: Only show stories from followed accounts or your own
  story_users = db.execute("""
    SELECT u.userId, u.username, u.profilePicture, COUNT(s.storyId) AS story_count
    FROM stories s
    JOIN users u ON u.userId = s.userId
    -- NEW: Join with follows table to filter only followed accounts
    LEFT JOIN follows f ON f.followingId = u.userId AND f.followerId = ?
    WHERE datetime(s.created_at) >= datetime('now', '-1 day')
      -- NEW: Only show stories from followed accounts OR your own stories
      AND (f.followerId IS NOT NULL OR u.userId = ?)
    GROUP BY u.userId
...
def delete_post(postId):
  ...
  if post and post['userId'] == session['userId']:
    # Delete dependent likes and comments FIRST (resolves foreign key error)
...
#edit_profile (UPDATED TO LOAD POSTS)
...
def edit_profile():
    ...
    # Load posts exactly like profile.html
    posts = db.execute("""
        SELECT p.*, COUNT(DISTINCT l.userId) AS like_count,
        COUNT(DISTINCT c.commentId) AS comment_count
        FROM   posts p
...
# NEW: Combined Followers + Following Page (for edit_profile button)
...
def follows(username):
  ...
  # Get BOTH followers and following lists
